...
class GridSearchCAP(CAPContingencyTable):
    ...
    def _evaluate(self, model: CAPContingencyTableQ):
        estim_tables = [
            model.predict_ct(sample.X, posteriors=P)
    ...
    def _prepare_classifier(self, cls_params):
        ...
        def job(cls_params):
            ...
            data = model.preprocess_data(self._training, posteriors=self._training_posteriors)
    ...
    def _compute_scores(self, training, posteriors=None):
        ...
        self._training_posteriors = posteriors
        if self.n_jobs == 0:
        ...
        # compute the self.h posteriors for the samples generated by self.protocol
        self.prot_posteriors = [get_posteriors_from_h(self.model.h, U_i.X) for U_i in self.protocol()]

        # compute the true accuracy values for the samples generated by self.protocol
        self.prot_true_accs = [
            self.model.true_acc(Ui, acc_fn=self.acc_fn, posteriors=P)
    ...
    def fit(self, training: LabelledCollection, posteriors=None):
        ...
        results = self._compute_scores(training, posteriors=posteriors)
    ...
    def predict_ct(self, test, posteriors=None, oracle_prev=None):
        ...
        return self.best_model().predict_ct(test, posteriors=posteriors, oracle_prev=oracle_prev)
