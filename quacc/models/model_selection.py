...
class GridSearchCAP(CAPContingencyTable):
    ...
    def __check_acc_fn(self, acc_fn):
        if acc_fn in qc.error.ACCURACY_MEASURE:
            ...
            self.error = qc.error.from_name(acc_fn)
    ...
    def _evaluate(self, model: CAPContingencyTableQ, error: Callable | str = qp.error.mae):
        if isinstance(error, str):
            error = qp.error.from_name(error)

        assert hasattr(error, "__call__"), "Invalid error function for evaluation"

        estim_tables = [model.predict_ct(sample) for sample in self.protocol()]
        estim_accs = np.array([self.error(cont_table) for cont_table in estim_tables]).reshape(-1, 1)

        return error(self.prot_true_accs, estim_accs)
    ...
    def _prepare_aggregation(self, args):
        ...
        def job(q_params):
            ...
            score = self._evaluate(model, protocol=self.protocol, error_measure=self.error)
            return score
    ...
    def _compute_scores(self, training):
        ...
        self.prot_true_accs = [true_acc(self.model.h, self.error, Ui) for Ui in self.protocol()]
