...
class GridSearchCAP(CAPContingencyTable):
    ...
    def _evaluate(self, model: CAPContingencyTableQ):
        estim_tables = [
            model.predict_ct(sample.X, P)
            for sample, P in IT.zip_longest(self.protocol(), self.prot_posteriors)
        ]
        estim_accs = np.array(
            [self.acc_fn(cont_table) for cont_table in estim_tables]
        ).reshape(-1, 1)
    ...
    def _compute_scores(self, training, training_posteriors):
        ...
        success_outs = []
        for (model, predictions, data, status, took), cls_config in zip(
            cls_outs, cls_configs
        ):
            if status.success():
        ...
        aggr_configs = [
            (*out, q_config) for out, q_config in IT.product(success_outs, q_configs)
        ]
        if self.n_jobs == 0:
    ...
    def _print_status(self, params, score, status, took):
        if status.success():
            self._sout(
                f"hyperparams=[{params}]\t got {self.error.__name__} = {score:.5f} [took {took:.3f}s]"
            )
        else:
...
def group_params(param_grid: dict):
    ...
    for key, values in param_grid.items():
        if (
            key.startswith("q_class__classifier__")
            or key.startswith("add_")
            or key == "q_class__val_split"
        ):
