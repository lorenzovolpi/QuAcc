...
class LabelledCollection(LC):
    ...
    def compact_classes(self):
        ...
        non_empty_collection = LabelledCollection(
            self.X, compact_y, classes=compact_classes
        )
...
class ContTableTransferCAP(CAPContingencyTableQ):
    ...
    def preprocess_data(self, data: LabelledCollection, posteriors):
        ...
        self.cont_table = confusion_matrix(
            y_true=y_true, y_pred=y_hat, labels=data.classes_, normalize="all"
        )
...
class QuAcc(CAPContingencyTableQ):
    ...
    def _safe_quantify(self, instances):
        ...
        if self._num_non_empty_classes() == 0 or num_instances == 0:
            ...
            uniform = np.full(
                fill_value=1.0 / self.q_n_classes, shape=self.q_n_classes, dtype=float
            )
            return uniform
...
class QuAcc1xNN(QuAcc):
    ...
    def _get_ct_hat(self, n, ct_compressed):
        _diag_idx = np.diag_indices(n)
        ct_rev_idx = (
            np.append(_diag_idx[0], (_diag_idx[0] + 1) % n),
            np.append(_diag_idx[1], _diag_idx[1]),
        )
        ct_hat = np.zeros((n, n))
        ct_hat[ct_rev_idx] = ct_compressed
        return ct_hat
...
class QuAccNxN(QuAcc):
    ...
    def _safe_q_classifier_fit_predict(self, compact_data: LabelledCollection):
        classif_predictions = []
        for q_i, data_i, num_nec_i in zip(
            self.q, compact_data, self._num_non_empty_classes()
        ):
            if num_nec_i <= 1:
            ...
            if isinstance(predict_on, int):
                if predict_on <= 1:
                    raise ValueError(
                        f"invalid value {predict_on} in fit. Specify a integer >1 for kFCV estimation."
                    )
                ...
                if not _invalid_split:
                    _split_n_classes = [
                        len(np.unique(data_i.y[train_idx]))
    ...
    def quant_classifier_fit_predict(self, data: list[LabelledCollection]):
        ...
        compact_data, self.q_old_class_idx = tuple(
            map(list, zip(*[data_i.compact_classes() for data_i in data]))
        )
    ...
    def _safe_quantify(self, instances_list):
        ...
        for X, q_i, num_nec_i, n_classes_i, qoci_i in zip(
            instances_list,
            self.q,
            self._num_non_empty_classes(),
            self.q_n_classes,
            self.q_old_class_idx,
        ):
            num_instances = X.shape[0]
            if num_nec_i == 0 or num_instances == 0:
                uniform = np.full(
                    fill_value=1.0 / n_classes_i, shape=n_classes_i, dtype=float
                )
    ...
    def predict_ct(self, X: LabelledCollection, posteriors, oracle_prev=None):
        ...
        cont_table = [
            p_i * cctp_i for p_i, cctp_i in zip(pred_prev, classcond_cond_table_prevs)
        ]
