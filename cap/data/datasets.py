...
def fetch_RCV1MulticlassDataset(target, train_val_split=0.5, include_zero=False):
    ...
    training = fetch_rcv1(subset="train", data_home=env["SKLEARN_DATA"])
    test = fetch_rcv1(subset="test", data_home=env["SKLEARN_DATA"])
    tr_labels = extend_labels(
        training.target.toarray(), training.target_names, class_names
    )
...
def fetch_twitterDataset(
    dataset_name, data_home=env["QUAPY_DATA"], train_val_split=0.5
):
    train, U = qp.datasets.fetch_twitter(
        dataset_name, min_df=3, pickle=True, data_home=data_home
    )
    T, V = split_train(train, train_val_split)
    return T, V, U


def fetch_UCIBinaryDataset(
    dataset_name, data_home=env["QUAPY_DATA"], train_val_split=0.5
):
