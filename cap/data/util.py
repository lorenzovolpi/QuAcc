...
def split_train(train: LabelledCollection, train_val_split: float):
    return train.split_stratified(
        train_prop=train_val_split, random_state=qp.environ["_R_SEED"]
    )


def get_rcv1_class_info():
    json_path = os.path.join(cap.env["CAP_DATA"], "rcv1_class_info.json")
    if not os.path.exists(json_path):
        # retrieve hierarchy file and class names
        hierarchy_tmp_path = os.path.join(cap.env["CAP_DATA"], "rcv1_hierarchy.tmp")
